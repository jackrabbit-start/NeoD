...
test('arena scene owns equipped-weapon lifecycle without moving presentation into combat behaviors', () => {
  ...
  const endRunBody = arenaSceneSource.match(/private endRun\(outcome: RunOutcome\): void \{(?<body>[\s\S]*?)\n  \}/)?.groups?.body ?? ''
  assert.match(endRunBody, /this\.destroyEquippedWeaponVisual\(\)/)
  assert.ok(endRunBody.indexOf('this.destroyEquippedWeaponVisual()') < endRunBody.indexOf("this.scene.start('result', payload)"))
})
