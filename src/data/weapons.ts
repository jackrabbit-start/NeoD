import type { WeaponDefinition, WeaponId } from '../domain/types.js'

export const WEAPON_DEFINITIONS: Record<WeaponId, WeaponDefinition> = {
  'starter-blaster': {
    id: 'starter-blaster',
    name: 'Bullet Hose',
    description: '짧은 박자로 탄막을 밀어 넣는 기관총입니다. 커뮤에서는 김동성이 파친코 대신 마우스 꾹 누르는 중이라고 놀립니다.',
    identityLabel: '지속 탄막',
    identityHint: '한 번 잡은 라인을 놓치지 않고 계속 긁어야 값이 나오는 기본 지속 화력 무기입니다.',
    range: 420,
    damage: 5,
    fireRateMs: 250,
    projectileSpeed: 760,
    projectileTint: 0xf8fafc,
    projectileTextureKey: 'starter-projectile',
    knockback: {
      force: 46,
      durationMs: 72,
    },
    attackBehavior: {
      kind: 'burst-fire',
      shotsPerBurst: 5,
      shotIntervalMs: 28,
      projectileLifetimeMs: 680,
      damageMultiplier: 0.68,
      speedMultiplier: 1.05,
      spreadDegrees: 3,
    },
    visual: {
      hudIconKey: 'weapon-starter-blaster',
      accentColor: 0xf8fafc,
    },
  },
  'acid-sprayer': {
    id: 'acid-sprayer',
    name: 'Spitter',
    description: '독액을 넓게 토해 바닥까지 오염시키는 범위 무기입니다. 토큰 잃은 날의 표정처럼 질척하게 남는다는 드립이 붙었습니다.',
    identityLabel: '산성 토사',
    identityHint: '직격보다 토해 놓은 오염 지대까지 묶어 적 무리를 태우는 광역 압박 무기입니다.',
    range: 240,
    damage: 11,
    fireRateMs: 420,
    projectileSpeed: 460,
    projectileTint: 0xc1ff72,
    projectileTextureKey: 'acid-projectile',
    knockback: {
      force: 58,
      durationMs: 86,
    },
    attackBehavior: {
      kind: 'spray-hazard',
      projectileCount: 3,
      spreadDegrees: 11,
      projectileLifetimeMs: 360,
      hazardRadius: 34,
      hazardDurationMs: 1400,
      hazardTickMs: 220,
      hazardDamage: 6,
    },
    visual: {
      hudIconKey: 'weapon-acid-sprayer',
      accentColor: 0xc1ff72,
    },
  },
  'frost-lance': {
    id: 'frost-lance',
    name: 'Rocket Forge',
    description: '무거운 로켓을 쏘아 도착 지점을 크게 날려버리는 폭발 무기입니다. 한 방에 판을 엎고 싶던 김동성의 파친코식 역전 욕망을 닮았습니다.',
    identityLabel: '폭심 유도',
    identityHint: '정면 직사보다 적이 몰린 지점에 꽂아 한 번에 날리는 범위 청소 무기입니다.',
    range: 520,
    damage: 16,
    fireRateMs: 760,
    projectileSpeed: 410,
    projectileTint: 0x9ce7ff,
    projectileTextureKey: 'frost-projectile',
    knockback: {
      force: 124,
      durationMs: 122,
    },
    attackBehavior: {
      kind: 'impact-aoe',
      projectileLifetimeMs: 1100,
      explosionRadius: 88,
      explosionDamage: 28,
    },
    visual: {
      hudIconKey: 'weapon-frost-lance',
      accentColor: 0x9ce7ff,
    },
  },
  'storm-cannon': {
    id: 'storm-cannon',
    name: 'Street Sweeper',
    description: '짧은 거리에서 한 번에 퍼져나가는 샷건입니다. 가까이 오면 말보다 먼저 터지는 커뮤식 손절 버튼 같은 무기입니다.',
    identityLabel: '근접 제압',
    identityHint: '붙어서 퍼뜨릴수록 여러 탄이 겹쳐 들어가며 잡몹 라인을 한 번에 비우는 산탄 무기입니다.',
    range: 180,
    damage: 10,
    fireRateMs: 520,
    projectileSpeed: 620,
    projectileTint: 0xd4b5ff,
    projectileTextureKey: 'storm-projectile',
    knockback: {
      force: 108,
      durationMs: 104,
    },
    attackBehavior: {
      kind: 'split-shot',
      projectileCount: 8,
      spreadDegrees: 9,
      projectileLifetimeMs: 260,
      damageMultiplier: 0.84,
      speedMultiplier: 0.96,
      maxHits: 1,
    },
    visual: {
      hudIconKey: 'weapon-storm-cannon',
      accentColor: 0xd4b5ff,
    },
  },
  'arc-loom': {
    id: 'arc-loom',
    name: 'Kickoff',
    description: '차 올린 공이 적 사이를 튕기며 연속 충돌하는 리바운드 무기입니다. 어릴 때 축구는 꽤 했다는 김동성의 오래된 허세가 무기화됐습니다.',
    identityLabel: '연쇄 킥',
    identityHint: '탄을 쏘는 게 아니라 튕김 경로를 태우는 무기라 적이 몰릴수록 공이 더 오래 살아남습니다.',
    range: 430,
    damage: 12,
    fireRateMs: 360,
    projectileSpeed: 560,
    projectileTint: 0xffd866,
    projectileTextureKey: 'arc-projectile',
    knockback: {
      force: 84,
      durationMs: 94,
    },
    attackBehavior: {
      kind: 'single',
      projectileLifetimeMs: 1600,
      ricochet: {
        maxBounces: 4,
        bounceRange: 210,
        damageMultiplierPerBounce: 0.94,
        speedMultiplierPerBounce: 1.03,
      },
    },
    visual: {
      hudIconKey: 'weapon-arc-loom',
      accentColor: 0xffd866,
    },
  },
  'spark-carbine': {
    id: 'spark-carbine',
    name: 'Sentry Node',
    description: '착지 지점에 감시 포탑을 세워 대신 사격하게 만드는 설치 무기입니다. 본인은 뛰고 감시는 기계에 맡기는 요즘식 생존 꼼수입니다.',
    identityLabel: '포탑 배치',
    identityHint: '직접 딜하는 시간이 아니라 포탑을 어디에 남기느냐가 전투 효율을 좌우하는 설치형 무기입니다.',
    range: 320,
    damage: 3,
    fireRateMs: 920,
    projectileSpeed: 340,
    projectileTint: 0xfff06a,
    projectileTextureKey: 'spark-projectile',
    knockback: {
      force: 44,
      durationMs: 70,
    },
    attackBehavior: {
      kind: 'deploy-turret',
      projectileLifetimeMs: 620,
      impactDamage: 3,
      speedMultiplier: 0.9,
      deploy: {
        maxTurrets: 2,
        durationMs: 5200,
        range: 250,
        fireRateMs: 420,
        projectileLifetimeMs: 700,
        projectileSpeed: 620,
        projectileDamage: 9,
      },
    },
    visual: {
      hudIconKey: 'weapon-spark-carbine',
      accentColor: 0xfff06a,
    },
  },
  'mist-vortex': {
    id: 'mist-vortex',
    name: 'Compiler',
    description: '바닥에 코드 비슷한 문장을 새기고 적이 오면 폭발시키는 함정형 무기입니다. 커뮤 댓글처럼 밟는 순간 터지는 문장 폭탄입니다.',
    identityLabel: '문법 폭발',
    identityHint: '적 위에 바로 꽂기보다 이동 경로에 문장을 심어 두고 밟게 만드는 설치 주문 무기입니다.',
    range: 280,
    damage: 8,
    fireRateMs: 440,
    projectileSpeed: 290,
    projectileTint: 0xc4f1ff,
    projectileTextureKey: 'mist-projectile',
    knockback: {
      force: 36,
      durationMs: 82,
    },
    attackBehavior: {
      kind: 'zone-control',
      projectileLifetimeMs: 520,
      zoneRadius: 44,
      zoneDurationMs: 1800,
      zoneTickMs: 210,
      zoneDamage: 8,
      speedMultiplier: 0.52,
    },
    visual: {
      hudIconKey: 'weapon-mist-vortex',
      accentColor: 0xc4f1ff,
    },
  },
  'slime-glaive': {
    id: 'slime-glaive',
    name: 'Blood Reaver',
    description: '적 한가운데 붙어 원형으로 갈아내며 체력을 회수하는 브루저 무기입니다. 레넥톤은 애정 가는 롤 캐릭터라며 들고 온 김동성식 흡혈 난전입니다.',
    identityLabel: '흡혈 난전',
    identityHint: '멀리서 깎기보다 적 무리 중앙을 버티며 여러 대상을 동시에 맞춰 회복을 챙기는 난전 무기입니다.',
    damage: 17,
    fireRateMs: 560,
    projectileSpeed: 0,
    projectileTint: 0x7dffb0,
    projectileTextureKey: 'arc-projectile',
    knockback: {
      force: 118,
      durationMs: 118,
    },
    attackBehavior: {
      kind: 'melee-cleave',
      range: 86,
      arcDegrees: 360,
      visualDurationMs: 170,
      maxTargets: 6,
      healOnHit: 2,
    },
    visual: {
      hudIconKey: 'weapon-slime-glaive',
      accentColor: 0x7dffb0,
    },
  },
  'prism-cutter': {
    id: 'prism-cutter',
    name: 'Iron Fist',
    description: '잽과 킥을 잇다가 마지막 피니시로 몰아치는 격투 콤보 무기입니다. 말로 해명하다 안 되면 몸이 먼저 나가는 짤 같은 연속기입니다.',
    identityLabel: '근접 콤보',
    identityHint: '한 번 휘두르고 끝나는 무기가 아니라 짧은 연속기 전체가 한 사이클이라 붙어 있을수록 이득을 봅니다.',
    damage: 14,
    fireRateMs: 560,
    projectileSpeed: 0,
    projectileTint: 0xff9df3,
    projectileTextureKey: 'frost-projectile',
    knockback: {
      force: 84,
      durationMs: 88,
    },
    attackBehavior: {
      kind: 'combo-melee',
      stepIntervalMs: 68,
      steps: [
        {
          damageMultiplier: 0.75,
          range: 58,
          arcDegrees: 42,
          visualDurationMs: 82,
          maxTargets: 1,
        },
        {
          damageMultiplier: 0.82,
          range: 60,
          arcDegrees: 46,
          visualDurationMs: 86,
          maxTargets: 1,
        },
        {
          damageMultiplier: 0.92,
          range: 68,
          arcDegrees: 58,
          visualDurationMs: 94,
          maxTargets: 2,
          knockbackMultiplier: 1.1,
        },
        {
          damageMultiplier: 1.55,
          range: 78,
          arcDegrees: 112,
          visualDurationMs: 118,
          maxTargets: 4,
          knockbackMultiplier: 1.6,
        },
      ],
    },
    visual: {
      hudIconKey: 'weapon-prism-cutter',
      accentColor: 0xff9df3,
    },
  },
  'needle-fan': {
    id: 'needle-fan',
    name: 'Reanimator',
    description: '이 무기로 처치한 적을 잠시 아군으로 되살리는 지배형 무기입니다. 탈출길에 구경꾼까지 알바로 세우는 김동성식 인력 운용입니다.',
    identityLabel: '시체 재가동',
    identityHint: '직접 지우는 데서 끝나지 않고 처치가 다음 병력으로 이어져 전투가 길수록 전장이 유리해집니다.',
    range: 380,
    damage: 13,
    fireRateMs: 360,
    projectileSpeed: 560,
    projectileTint: 0xffb15c,
    projectileTextureKey: 'needle-projectile',
    knockback: {
      force: 72,
      durationMs: 84,
    },
    attackBehavior: {
      kind: 'single',
      projectileLifetimeMs: 760,
      summonOnKill: {
        maxMinions: 3,
        durationMs: 4200,
        speed: 170,
        damage: 10,
        attackIntervalMs: 320,
        contactRadius: 24,
      },
    },
    visual: {
      hudIconKey: 'weapon-needle-fan',
      accentColor: 0xffb15c,
    },
  },
}
